		...
		if(root)
		{
			...
			else if(root->key=='+' || root->key=='-' || root->key=='*' || root->key=='/')
			{
				...
				if(root->left->label==0 || root->right->label==0)
				{
					...
					if(temp->flag==1)
					{
						cout<<"R"<<temp->number<<","<<root->right->key<<";"<<endl;
						temp->flag=root->flag;
						st.push(temp);
					}
					else
					{
						cout<<root->left->key<<","<<"R"<<temp->number<<";"<<endl;
						temp->flag=root->flag;
						st.push(temp);
					}
				}
				...
				else
				{
					...
					if(temp1->flag==1)
					{
						cout<<"R"<<temp1->number<<","<<"R"<<temp2->number<<";"<<endl;
					}
					else
					{
						cout<<"R"<<temp2->number<<","<<"R"<<temp1->number<<";"<<endl;
					}
